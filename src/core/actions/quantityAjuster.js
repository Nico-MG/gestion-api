...
export default async function quantityAdjuster(tabla, accion, detalles, detallesOriginal) {
	...
	for (let idx = 0; idx < detalles.length; idx++) {
		...
		if ((tabla !== "SAL" && accion === "ADD") || (tabla === "SAL" && accion === "DEL")) {
			productoValido.cit += detalles[idx].quantity;
		}
		if ((tabla !== "SAL" && accion === "DEL") || (tabla === "SAL" && accion === "ADD")) {
			productoValido.cit -= detalles[idx].quantity;
		}
